 ...
 * template system
 * usage:
   var tmpl = new gbif.core.Template({
...
gbif.core.Template = Backbone.Model.extend({
  ...
  compile: function() {
    ...
    var fn = gbif.core.Template.compilers[tmpl_type];
    if(fn) {
      return fn(this.get('template'));
    } else {
      gbif.log.error("can't get rendered for " + tmpl_type);
    }
    return null;
  },
  ...
  */
  render: function(vars) {
    ...
    var r = gbif.core.Profiler.get('template_render');
    r.start();
    var rendered = c(vars);
    r.end();
    return rendered;
...
gbif.core.TemplateList = Backbone.Collection.extend({

  model: gbif.core.Template,
  ...
  getTemplate: function(template_name) {
    ...
    gbif.log.error(template_name+" not found");
    return null;
  }
});
...
* global variable
*/
gbif.templates = new gbif.core.TemplateList();
...
*/
gbif._loadJST = function() {
  if(typeof(window.JST) !== undefined) {
    gbif.templates.reset(
